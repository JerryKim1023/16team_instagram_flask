...
def search_db():
    ...
    mypage_search = db.mypage.find(
        {"title": {"$regex": f".*{input}.*"}}, {"_id": False})
    return jsonify({'insta_search': list(rest_search) + list(mypage_search)}), 200
...
@app.route("/api/show_post_main_user", methods=['GET'])
def show_user_info():
    token_receive = request.cookies.get('mytoken')
    payload = jwt.decode(token_receive, SECRET_KEY, algorithms=['HS256'])  # jwt decode
    print(payload)
...
def file_show():
        ...
        return render_template('feedindex.html')
    # return render_template('mypage.html', img=img_binaries, user=user_info["id"])
